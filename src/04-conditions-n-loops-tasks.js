...
function doRectanglesOverlap(rect1, rect2) {
  const contains = (rect, x, y) => rect.top <= y && rect.top + rect.height >= y
    && rect.left <= x && rect.left + rect.width >= x;
  return contains(rect1, rect2.left, rect2.top)
    || contains(rect1, rect2.left + rect2.width, rect2.top)
    || contains(rect1, rect2.left, rect2.top + rect2.height)
    || contains(rect1, rect2.left + rect2.width, rect2.top + rect2.height)
    || contains(rect2, rect1.left, rect1.top)
    || contains(rect2, rect1.left + rect1.width, rect1.top)
    || contains(rect2, rect1.left, rect1.top + rect1.height)
    || contains(rect2, rect1.left + rect1.width, rect1.top + rect1.height);
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = number.length - 1; i >= 0; i -= 1) {
    const digit = Number.parseInt(number[i], 10);
    if (index % 2 === 0) {
      result += digit;
    } else if (digit < 5) {
      result += digit * 2;
...
function getCommonDirectoryPath(pathes) {
  ...
  let result = pathes[0].split('/').slice(0, -1);
  for (let i = 1; i < pathes.length; i += 1) {
    const segments = pathes[i].split('/').slice(0, -1);
    let commonLength = 0;
    for (; commonLength < Math.min(result.length, segments.length); commonLength += 1) {
      if (result[commonLength] !== segments[commonLength]) {
        break;
      }
    }
    if (commonLength !== 0) {
      result = result.slice(0, commonLength);
    } else {
      result = [];
    }
  }
